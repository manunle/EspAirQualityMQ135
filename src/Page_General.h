...
<tr>
  <td align="right">RZERO</td>
  <td><input type="text" id="vRZERO" name="vRZERO" value=""></td>
</tr>
...
void send_devicename_value_html()
{
  ...
  values += "vRZERO|" + (String) config.vRZERO + "|div\n";
...
void send_general_html()
{
		...
		for ( uint8_t i = 0; i < server.args(); i++ ) 
    {
      ...
      if (server.argName(i) == "vRZERO") config.vRZERO = server.arg(i).toFloat();
		}
		WriteConfig();
		firstStart = true;
	}
...
void send_general_configuration_values_html()
{
  ...
  values += "vRZERO|" + (String) config.vRZERO + "|input\n";
	server.send ( 200, "text/plain", values);
	Serial.println(__FUNCTION__); 
